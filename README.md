# Minecraft: Drakonix One Block Shop

## Overview
challenge mod where you start in a 1x1 block vertical border

you have to sell items to expand your vertical border

you start with a "Drakonix One Block Shop" block (that, if right-clicked, has a GUI that
lets you sell items) and an iron pickaxe
- you can hopper items into it to auto-sell
- it also has a GUI that lets you buy items at a high price if you can't produce them on your own
  - i.e. sugarcane, lava buckets, etc


you start with a guide (a written book) populated from GUIDE.md
- it suggests auto farms like mob farms, fishing, sugarcane redstone, and cactus redstone


## Running

    ./gradlew runClient
